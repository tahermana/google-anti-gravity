...
class AppState extends ChangeNotifier {
  ...
  void recalculateGoal() {
    ...
    final age = now.year - birthYear - ((now.month < birthMonth ||
        (now.month == birthMonth && now.day < birthDay)) ? 1 : 0);
  ...
  final List<Meal> meals = [
    const Meal(name: 'Oatmeal with banana', type: 'Breakfast', time: '8:15 AM', kcal: 380, emoji: '🥣', protein: 12, carbs: 68, fat: 6),
    const Meal(name: 'Black coffee',        type: 'Breakfast', time: '8:30 AM', kcal: 5,   emoji: '☕', protein: 0,  carbs: 0,  fat: 0),
    const Meal(name: 'Grilled chicken salad', type: 'Lunch',  time: '1:00 PM', kcal: 420, emoji: '🥗', protein: 45, carbs: 22, fat: 14),
    const Meal(name: 'Whole grain bread',   type: 'Lunch',    time: '1:05 PM', kcal: 180, emoji: '🍞', protein: 6,  carbs: 34, fat: 2),
    const Meal(name: 'Mixed nuts',          type: 'Snack',    time: '3:30 PM', kcal: 175, emoji: '🥜', protein: 5,  carbs: 7,  fat: 15),
  ];
  ...
  void toggleAi()        { aiEnabled = !aiEnabled;               notifyListeners(); }
  void toggleDarkMode()  { darkMode = !darkMode;                 notifyListeners(); }

  // Weekly calorie data (Mon–Sun)
  final List<double> weeklyCalories = [1920, 2100, 1750, 1980, 2050, 1840, 1160];
